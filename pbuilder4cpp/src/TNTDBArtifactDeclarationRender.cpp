...
public:
    ...
    std::string str = cdn;
    boost::replace_all(str, "TABLE", pbuilder::StringUtils::toUpper(table_.applicationName));
...
void TNTDBArtifactDeclarationRender::tableExtended(const pbuilder::Table & table_) {
    ...
    if (table_.pkColumns.size() > 1) {
        keytype = render->parent->pbuilder->unit.ns + "::entity::" + pbuilder::StringUtils::toUpper(table_.applicationName) + "Id";
